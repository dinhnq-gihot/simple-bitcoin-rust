use {
    ...
    btclib::{crypto::PublicKey, network::Message, types::Block, util::Saveable},
    clap::Parser,
    std::{
        sync::{
            Arc,
...
struct Cli {
    ...
    address: String,
    #[arg(short, long, default_value = "./accounts/alice.pub.pem")]
...
impl Miner {
    async fn new(address: String, public_key: PublicKey) -> Result<Self> {
        ...
        let current_template = Arc::new(std::sync::Mutex::new(None));
        ...
        Ok(Self {
            ...
            current_template,
            mining,
            mined_block_sender: sender,
            mined_block_receiver: receiver,
        })
    }

    async fn run(&self) -> Result<()> {
        self.spawn_mining_thread();
        let mut template_interval = tokio::time::interval(Duration::from_secs(5));
        loop {
    ...
    async fn fetch_template(&self) -> Result<()> {
        ...
        let message = Message::FetchTemplate(self.public_key);
